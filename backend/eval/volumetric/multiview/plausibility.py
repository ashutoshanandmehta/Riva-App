"""Plausibility gate — REQUIRED before any value is logged.
...
Ranges live in food_classes.json (per class: volume_ml, density_g_ml, kcal_100g);
unknown classes use `_generic`. Nothing is ever silently logged at the raw value.
"""
...
def gate(raw_volume_ml: float, food_name: str | None, base_confidence: float) -> GateResult:
    key, rec = resolve_class(food_name)
    lo, hi = rec["volume_ml"]
    ...
    if raw_volume_ml < lo / RETAKE_FACTOR or raw_volume_ml > hi * RETAKE_FACTOR:
        return GateResult(key, "retake", raw_volume_ml, raw_volume_ml, None, None,
                          0.0, f"volume {raw_volume_ml:.0f} mL is >{RETAKE_FACTOR:g}x "
